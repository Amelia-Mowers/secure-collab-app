...
- [~] **Make E2E encryption real — or stop claiming it** — `[§4.2]` — _enablement + guard + honest badge done & compile-verified; encrypted send/decrypt round-trip now **verified against Conduit** (`test_two_client_encrypted_round_trip`). Remaining before launch: device verification + cross-signing + key backup._
  ...
  - [~] **Device verification + cross-signing + key backup** — design recorded in **`docs/adr/0001-e2e-key-management.md`** (test-first plan). Done so far: a **red spec test** (`test_second_device_reconstructs_encrypted_workspace_from_history`, behind the `red-tests` feature — a second device can't reconstruct an encrypted workspace yet), and **undecryptable-history detection** (`MatrixClient::is_undecryptable_event` + cold-start/sync counting + `ConnectedWorkspace::undecryptableCount()`) so the gap is no longer silent. Remaining: `EncryptionSettings` auto-enable, recovery-key UX, verification UX, and the UI warning banner.
