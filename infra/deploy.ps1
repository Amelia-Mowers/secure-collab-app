...
ssh -o BatchMode=yes $Target "mkdir -p /srv/tidework/deploy && rm -rf /srv/tidework/deploy/synapse /srv/tidework/deploy/mas /srv/tidework/deploy/nginx /srv/tidework/deploy/systemd /srv/tidework/deploy/ssh"
scp -q "$here\docker-compose.yml" "$here\remote-setup.sh" "$here\healthcheck.sh" "$here\db-ca.crt" "${Target}:/srv/tidework/deploy/"
scp -qr "$here\synapse" "$here\mas" "$here\nginx" "$here\systemd" "$here\ssh" "${Target}:/srv/tidework/deploy/"
